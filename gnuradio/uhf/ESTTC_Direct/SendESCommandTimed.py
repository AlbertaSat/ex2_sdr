...
addr_tx = ("127.0.0.1", 52001)
addr_rx = ("127.0.0.1", 52002)
...
def readSCW():
    ...
    readSCWMsg = b"ES+R2200 BD888E1F"+b"\r"
    return readSCWMsg
...
def main():
    ...
    try:
        client_socket_tx.connect(addr_tx)
        client_socket_rx.connect(addr_rx)
    except:
    ...
    if result['esttc_msg'] != 'Quit':
        ...
        client_socket_rx.settimeout(txIntervalMillis/1000) # Timeout equals sleep time to preserve total duration
        ...
        while txDuration > 0:
            ...
            print("sending packet "+str(txPacketCount)+"...")
            client_socket_tx.sendto(thePacket, addr_tx)
            
            try:
                theResponse = client_socket_rx.recv(256)
                theResponse = theResponse.decode(encoding='ascii', errors='ignore')

                if(theResponse == ''):
                    print("No ascii founds in packet - must have received noise")
                else:
                    theResponse = theResponse.replace('\r', '').replace('\n','')
                    print("Received response " + theResponse)
                    rxPacketCount += 1
        ...
        print("Received {} packets, expected {}".format(rxPacketCount, txPacketCount))
    else:
        print("No command will be sent")
    
    client_socket_rx.shutdown(socket.SHUT_RDWR)
    client_socket_tx.shutdown(socket.SHUT_RDWR)

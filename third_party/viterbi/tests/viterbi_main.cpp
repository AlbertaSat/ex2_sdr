...
void ViterbiMain(const std::vector<std::string>& args)
{
    ...
    if (constraint <= 0) {
        std::cout << "Constraint should be greater than 0, found " << constraint << std::endl;
        exit(1);
    }
    ...
    for (int i = 1; i < args.size() - 1; i++) {
        ...
        if (polynomial <= 0) {
            std::cout << "Polynomial should be greater than 0, found " << polynomial << std::endl;
            exit(1);
        }
        if (polynomial >= (1 << constraint)) {
            std::cout << "Polynomial should be less than " << (1 << constraint) << ", found "
                      << polynomial << std::endl;
            exit(1);
        }
